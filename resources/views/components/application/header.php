<div class="fixed group w-72 h-full top-0 -left-72 sm:left-0 m-0 z-20 overflow-hidden transition-all border-r border-gray-200 bg-white dark:border-none dark:bg-neutral-800 shadow">
    <!-- Logo -->
    <div class="py-7 px-5">
    ...
    <!-- Main Menu -->
    <div class="absolute w-full left-0 transition-all overflow-x-hidden overflow-y-auto max-h-[calc(100vh-164px)] px-5">
        <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?>?page=beta" class="px-4 h-10 w-full transition-all mb-2 flex items-center no-underline cursor-pointerpy-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 <?= ($title === "Dashboard" ? 'bg-gray-100 dark:bg-neutral-700 font-medium' : '') ?>">
            <div class="w-4 h-4 align-top mr-5 flex items-center justify-center no-underline">
                <i class="fa-solid fa-gauge text-gray-800 dark:text-white"></i>
            </div>
            <div class="inline-block">
                <h1 class="font-baloo font-normal m-0 text-base leading-5 text-gray-800 dark:text-white">Dashboard</h1>
            </div>
        </a>
        <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?>?page=media" class="px-4 h-10 w-full transition-all mb-2 flex items-center no-underline cursor-pointerpy-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 <?= ($title === "Media" ? 'bg-gray-100 dark:bg-neutral-700 font-medium' : '') ?>">
            <div class="w-4 h-4 align-top mr-5 flex items-center justify-center no-underline">
                <i class="fa-solid fa-photo-film text-gray-800 dark:text-white"></i>
            </div>
            <div class="inline-block">
                <h1 class="font-baloo font-normal m-0 text-base leading-5 text-gray-800 dark:text-white">Media</h1>
            </div>
        </a>
        <div class="my-4 mx-auto block h-0 w-11/12 no-underline border-b border-gray-200 overflow-hidden"></div>
        <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?>?page=media" class="px-4 h-10 w-full transition-all mb-2 flex items-center no-underline cursor-pointerpy-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 <?= ($title === "Media" ? 'bg-gray-100 dark:bg-neutral-700 font-medium' : '') ?>">
            <div class="w-4 h-4 align-top mr-5 flex items-center justify-center no-underline">
                <i class="fa-solid fa-file text-gray-800 dark:text-white"></i>
            </div>
            <div class="inline-block">
                <h1 class="font-baloo font-normal m-0 text-base leading-5 text-gray-800 dark:text-white">Pages</h1>
            </div>
        </a>
        <?php if ($this->getModuleCustomMenuItems()): ?>
            <div class="my-4 mx-auto block h-0 w-11/12 no-underline border-b border-gray-200 overflow-hidden"></div>
        <?php endif; ?>
        <?php foreach ($this->getModuleCustomMenuItems() as $custom_menu_item): ?>

            <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?><?= $custom_menu_item['slug']; ?>" class="px-4 h-10 w-full transition-all mb-2 flex items-center no-underline cursor-pointerpy-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 <?= ($title === $custom_menu_item['name'] ? 'bg-gray-100 dark:bg-neutral-700 font-medium' : '') ?>">
                <div class="w-4 h-4 align-top mr-5 flex items-center justify-center no-underline">
                    <i class="fa-solid fa-gauge text-gray-800 dark:text-white"></i>
                </div>
                <div class="inline-block">
                    <h1 class="font-baloo font-normal m-0 mt-1 text-base leading-5 text-gray-800 dark:text-white"><?= $custom_menu_item['name'] ?></h1>
                </div>
            </a>
        <?php endforeach; ?>
        <div class="my-4 mx-auto block h-0 w-11/12 no-underline border-b border-gray-200 overflow-hidden"></div>
        <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?>?page=users" class="px-4 h-10 w-full transition-all mb-2 flex items-center no-underline cursor-pointerpy-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 <?= ($title === "Users" ? 'bg-gray-100 dark:bg-neutral-700 font-medium' : '') ?>">
            <div class="w-4 h-4 align-top mr-5 flex items-center justify-center no-underline">
                <i class="fa-solid fa-users text-gray-800 dark:text-white"></i>
            </div>
            <div class="inline-block">
                <h1 class="font-baloo font-normal m-0 mt-1 text-base leading-5 text-gray-800 dark:text-white">Users</h1>
            </div>
        </a>
        <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?>?page=modules" class="px-4 h-10 w-full transition-all mb-2 flex items-center no-underline cursor-pointerpy-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 <?= ($title === "Modules" ? 'bg-gray-100 dark:bg-neutral-700 font-medium' : '') ?>">
            <div class="w-4 h-4 align-top mr-5 flex items-center justify-center no-underline">
                <i class="fa-solid fa-plug text-gray-800 dark:text-white"></i>
            </div>
            <div class="inline-block">
                <h1 class="font-baloo font-normal m-0 mt-1 text-base leading-5 text-gray-800 dark:text-white">Modules</h1>
            </div>
        </a>
        <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?>?page=settings" class="px-4 h-10 w-full transition-all mb-2 flex items-center no-underline cursor-pointerpy-2 rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 <?= ($title === "Settings" ? 'bg-gray-100 dark:bg-neutral-700 font-medium' : '') ?>">
            <div class="w-4 h-4 align-top mr-5 flex items-center justify-center no-underline">
                <i class="fa-solid fa-gear text-gray-800 dark:text-white"></i>
            </div>
            <div class="inline-block">
                <h1 class="font-baloo font-normal m-0 mt-1 text-base leading-5 text-gray-800 dark:text-white">Settings</h1>
            </div>
        </a>
    </div>
    <!-- Account -->
        ...
        <div class="flex items-center justify-between px-5 py-4">
            <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col">
                    <h1 class="font-baloo font-semibold m-0 text-lg leading-5 text-gray-800 dark:text-white">
                        <?php $user = $this->getCurrentUser();
                        echo $this->e($user['firstname']) . ' ' . $this->e($user['lastname']); ?>
                    </h1>
                    <p class="flex gap-2 font-baloo font-normal m-0 text-base leading-4 underline text-gray-500 dark:text-white">
                        <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?>?page=account">Account</a>
                        <a href="<?= $this->e($this->getStrings()->getAdminFilePath()); ?>?page=logout">Logout</a>
                    </p>
                </div>
            </div>
        </div>
