    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if (isset($_POST['doLogin'])) {
            ...
            try {
                ...
                $cookieValue = $this->app->getAuth('websitesql')->authenticate($email, $password);

                // Return an redirect response
                $response = new RedirectResponse($this->app->getRouter()->getRoute('admin.dashboard'));
        ...
        $body = $this->app->getRenderer()->render('application::login', [
            'title' => 'Login',
            'error' => $error ?? null
        ]);	
