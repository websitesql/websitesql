...
namespace WebsiteSQL\WebsiteSQL\Controllers\Http\Api\Users;
...
class PostController implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        try {
            ...
            $role = isset($body['role']) ? filter_var($body['role'], FILTER_SANITIZE_NUMBER_INT) : 0;
            $approved = isset($body['approved']) ? filter_var($body['approved'], FILTER_SANITIZE_NUMBER_INT) : 0;
            $email_verified = isset($body['email_verified']) ? filter_var($body['email_verified'], FILTER_SANITIZE_NUMBER_INT) : 0;
            ...
            // If role is passed, check if the role is valid
            if ($role > 0)
            {
                $roleData = $this->app->getDatabase()->get($this->app->getStrings()->getTableRoles(), '*', ['id' => $role]);
                if (!$roleData)
                {
