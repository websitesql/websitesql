...
class RoutingProvider
{
    ...
    private function initSetupRoutes(): void
    {
        ...
        $this->registerAppRoute(['GET', 'POST'], '/setup', [\WebsiteSQL\WebsiteSQL\Http\Setup\SetupController::class, 'handle'], 'setup', []);
    }
    ...
    private function initAppRoutes(): void
    {
        ...
        $this->registerAppRoute(['GET', 'POST'], '/login', [\WebsiteSQL\WebsiteSQL\Http\Auth\LoginController::class, 'handle'], 'login', [
            'auth' => false
        ]);

        // Register the register route
        $this->registerAppRoute(['GET', 'POST'], '/register', [\WebsiteSQL\WebsiteSQL\Http\Auth\RegisterController::class, 'handle'], 'register', [
            'auth' => false
        ]);

        // Register the dashboard route
        $this->registerAppRoute('GET', '/dashboard', [\WebsiteSQL\WebsiteSQL\Http\App\DashboardController::class, 'handle'], 'dashboard', [
            'auth' => true,
            'app_access' => true
        ]);

        // Register the roles route
        $this->registerAppRoute(['GET', 'POST'], '/media', [\WebsiteSQL\WebsiteSQL\Http\App\DashboardController::class, 'handle'], 'media', [
            'auth' => true,
            'app_access' => true,
        ...
        $this->registerAppRoute('GET', '/updates', [\WebsiteSQL\WebsiteSQL\Http\App\UpdateController::class, 'handle'], 'updates', [
            'permissions' => 'wsql.updates.read'
        ]);

        // Register route: Account
        $this->registerAppRoute('GET', '/account', [\WebsiteSQL\WebsiteSQL\Http\App\AccountController::class, 'handle'], 'account', [
        ...
        $this->registerAppRoute('GET', '/admin/settings', [\WebsiteSQL\WebsiteSQL\Http\App\Admin\SettingsController::class, 'handle'], 'admin.settings', [
            'app_access' => true,
            'permissions' => 'wsql.settings.read'
        ]);

        // Register route: Allows access to view the user settings page
        $this->registerAppRoute('GET', '/admin/users', [\WebsiteSQL\WebsiteSQL\Http\App\Admin\UsersController::class, 'handle'], 'admin.users', [
            'app_access' => true,
            'permissions' => 'wsql.settings.users.read'
        ]);

        // Register route: Allows access to view the user settings page
        $this->registerAppRoute('GET', '/admin/users/{id:uuid}', [\WebsiteSQL\WebsiteSQL\Http\App\Admin\Single\UserController::class, 'handle'], 'settings.users.update', [
            'app_access' => true,
            'permissions' => 'wsql.settings.users.update'
        ]);

        // Register route: Allows access to view the access control settings page
        $this->registerAppRoute('GET', '/admin/access-control', [\WebsiteSQL\WebsiteSQL\Http\App\Admin\AccessControlController::class, 'handle'], 'admin.access-control', [
            'app_access' => true,
            'permissions' => 'wsql.settings.access-control.read'
        ]);

        // Register route: Allows access to edit access control settings
        $this->registerAppRoute('GET', '/admin/access-control/{id:uuid}', [\WebsiteSQL\WebsiteSQL\Http\App\Admin\Single\AccessControlController::class, 'handle'], 'settings.access-control.update', [
            'app_access' => true,
            'permissions' => 'wsql.settings.access-control.update'
        ]);

        // Register route: Allows access to update the settings
        $this->registerAppRoute('GET', '/admin/modules', [\WebsiteSQL\WebsiteSQL\Http\App\Admin\ModulesController::class, 'handle'], 'admin.modules', [
            'app_access' => true,
            'permissions' => 'wsql.settings.modules.read'
        ]);

        // Register route: Allows access to update the modules
        $this->registerAppRoute('POST', '/admin/modules', [\WebsiteSQL\WebsiteSQL\Http\App\Admin\ModulesController::class, 'handle'], 'settings.modules.post', [
            'app_access' => true,
    ...
    private function initApiRoutes(): void
    {
        ...
        $this->registerApiRoute('POST', '/media/upload', [\WebsiteSQL\WebsiteSQL\Http\Api\Media\Upload\PostController::class, 'handle'], 'media.upload.post', [
            'permissions' => 'wsql.api.media.upload'
        ]);


        // Register the settings routes
        $this->registerApiRoute('PATCH', '/settings/logging', [\WebsiteSQL\WebsiteSQL\Http\Api\Settings\Logging\PatchController::class, 'handle'], 'settings.logging.patch', [
            'permissions' => 'wsql.api.settings.logging.update'
        ]);

        // Register Route: Customizations GET
        $this->registerApiRoute('GET', '/customizations', [\WebsiteSQL\WebsiteSQL\Http\Api\Customizations\GetController::class, 'handle'], 'customizations.get', [
            'permissions' => 'wsql.api.customizations.read'
        ]);

        // Register Route: Settings Branding GET
        $this->registerApiRoute('GET', '/settings/branding', [\WebsiteSQL\WebsiteSQL\Http\Api\Settings\Branding\GetController::class, 'handle'], 'settings.branding.get', [
            'permissions' => 'wsql.api.settings.branding.read'
        ]);
        
        // Register Route: Settings Branding PATCH
        $this->registerApiRoute('PATCH', '/settings/branding', [\WebsiteSQL\WebsiteSQL\Http\Api\Settings\Branding\PatchController::class, 'handle'], 'settings.branding.patch', [
        ...
        $this->registerApiRoute('GET', '/users', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\GetController::class, 'handle'], 'users.get', [
            'permissions' => 'wsql.api.users.read'
        ]);

        // Register Route: Users POST (PLA)
        $this->registerApiRoute('POST', '/users', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\PostController::class, 'handle'], 'users.post', [
            'permissions' => 'wsql.api.users.create'
        ]);

        // Register Route: Users Register POST
        $this->registerApiRoute('POST', '/users/register', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\Register\PostController::class, 'handle'], 'users.register.post', [
            'auth' => false
        ]);

        // Register Route: Users Me GET
        $this->registerApiRoute('GET', '/users/me', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\Me\GetController::class, 'handle'], 'users.me.get', [
            'permissions' => 'wsql.api.users.me.read'
        ]);

        // Register Route: Users Me PATCH
        $this->registerApiRoute('PATCH', '/users/me', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\Me\PatchController::class, 'handle'], 'users.me.patch', [
            'permissions' => 'wsql.api.users.me.update'
        ]);

        // Register Route: Users Me Reset Password POST
        $this->registerApiRoute('POST', '/users/me/reset-password', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\Me\ResetPassword\PostController::class, 'handle'], 'users.me.reset-password.post', [
            'permissions' => 'wsql.api.users.me.reset-password'
        ]);

        // Register Route: User GET (PLA)
        $this->registerApiRoute('GET', '/users/{id:uuid}', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\Single\GetController::class, 'handle'], 'users.single.get', [
            'permissions' => 'wsql.api.users.single.read'
        ]);

        // Register Route: User PATCH (PLA)
        $this->registerApiRoute('PATCH', '/users/{id:uuid}', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\Single\PatchController::class, 'handle'], 'users.patch', [
            'permissions' => 'wsql.api.users.single.update'
        ]);

        // Register Route: User DELETE (PLA)
        $this->registerApiRoute('DELETE', '/users/{id:uuid}', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\Single\DeleteController::class, 'handle'], 'users.delete', [
            'permissions' => 'wsql.api.users.single.delete'
        ]);

        // Register Route: User Reset Password POST
        $this->registerApiRoute('POST', '/users/{id:uuid}/reset-password', [\WebsiteSQL\WebsiteSQL\Http\Api\Users\ResetPassword\PostController::class, 'handle'], 'users.reset-password.post', [
        ...
        $this->registerApiRoute('GET', '/roles', [\WebsiteSQL\WebsiteSQL\Http\Api\Roles\GetController::class, 'handle'], 'roles.get', [
            'permissions' => 'wsql.api.roles.read'
        ]);

        // Register Route: Roles POST (PLA)
        $this->registerApiRoute('POST', '/roles', [\WebsiteSQL\WebsiteSQL\Http\Api\Roles\PostController::class, 'handle'], 'roles.post', [
            'permissions' => 'wsql.api.roles.create'
        ]);

        // Register Route: Role GET (PLA)
        $this->registerApiRoute('GET', '/roles/{id:uuid}', [\WebsiteSQL\WebsiteSQL\Http\Api\Roles\Single\GetController::class, 'handle'], 'roles.single.get', [
            'permissions' => 'wsql.api.roles.single.read'
        ]);

        // Register Route: Role PATCH (PLA)
        $this->registerApiRoute('PATCH', '/roles/{id:uuid}', [\WebsiteSQL\WebsiteSQL\Http\Api\Roles\Single\PatchController::class, 'handle'], 'roles.single.patch', [
            'permissions' => 'wsql.api.roles.single.update'
        ]);

        // Register Route: Role DELETE (PLA)
        $this->registerApiRoute('DELETE', '/roles/{id:uuid}', [\WebsiteSQL\WebsiteSQL\Http\Api\Roles\Single\DeleteController::class, 'handle'], 'roles.single.delete', [
            'permissions' => 'wsql.api.roles.single.delete'
        ]);

        // Register Route: Role Permissions POST
        $this->registerApiRoute('POST', '/roles/{id:uuid}/permissions', [\WebsiteSQL\WebsiteSQL\Http\Api\Roles\Single\Permissions\PostController::class, 'handle'], 'roles.single.permissions.post', [
            'permissions' => 'wsql.api.roles.single.permissions.create'
        ]);

        // Register Route: Role Permissions GET
        $this->registerApiRoute('GET', '/roles/{id:uuid}/permissions', [\WebsiteSQL\WebsiteSQL\Http\Api\Roles\Single\Permissions\GetController::class, 'handle'], 'roles.single.permissions.get', [
            'permissions' => 'wsql.api.roles.single.permissions.read'
        ]);

        // Register Route: Role Permissions DELETE
        $this->registerApiRoute('DELETE', '/roles/{id:uuid}/permissions/{permission_id:number}', [\WebsiteSQL\WebsiteSQL\Http\Api\Roles\Single\Permissions\Single\DeleteController::class, 'handle'], 'roles.single.permissions.single.delete', [
        ...
        $this->registerApiRoute('GET', '/permissions', [\WebsiteSQL\WebsiteSQL\Http\Api\Permissions\GetController::class, 'handle'], 'permissions.get', [
        ...
        $this->registerApiRoute('POST', '/auth/logout', [\WebsiteSQL\WebsiteSQL\Http\Api\Auth\Logout\PostController::class, 'handle'], 'auth.logout.post', [
            'auth' => true
        ]);
    }
}